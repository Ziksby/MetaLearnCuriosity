            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_states, batch_info):
                    ...
                    def _fast_loss_fn(fast_params, traj_batch):
                        # The NLL Loss
                        fast_pi_st = fast.apply(fast_params, traj_batch.obs)
                        # fast_pi_st_1 = fast.apply(fast_params, traj_batch.next_obs)
                        loss = -fast_pi_st.log_prob(traj_batch.action)
                        # loss = jnp.linalg.norm((fast_pi_st_1.logits - fast_pi_st.logits), axis=1)
                        return loss.mean()

                    def _rl_loss_fn(params, traj_batch, gae, targets):
                        # RERUN NETWORK
                        pi, value, int_value = network.apply(params, traj_batch.obs)
