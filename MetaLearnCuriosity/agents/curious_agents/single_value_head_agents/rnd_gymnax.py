...
from MetaLearnCuriosity.utils import make_obs_gymnax_discrete,update_obs_norm_params, ObsNormParams, RNDTransition, RNDNormIntReturnParams, rnd_normalise_int_rewards

from MetaLearnCuriosity.checkpoints import Save
from MetaLearnCuriosity.logger import WBLogger
from MetaLearnCuriosity.wrappers import FlattenObservationWrapper, LogWrapper, VecEnv
from MetaLearnCuriosity.agents.nn import TargetNetwork, PredictorNetwork
...
def train(rng, train_state, pred_state, target_params, init_obs_rng):
    ...
    random_rollout = make_obs_gymnax_discrete(config["NUM_ENVS_PER_DEVICE"], env, env_params,config["NUM_STEPS"])

    # Obs will be in shape: num_steps, num_envs, obs.shape
    init_obs = random_rollout(init_obs_rng)
    init_obs = init_obs.reshape(-1, init_obs.shape[-1]) # reshape it to num_envs*num_steps, obs.shape
    ...
    rnd_int_return_norm_params=RNDNormIntReturnParams(1e-4,0.0,1.0,jnp.zeros((config["NUM_STEPS"],)))

    obs_norm_params = update_obs_norm_params(
        init_obs_norm_params,init_obs
    )
    target = TargetNetwork(64)
    ...
    def _update_step(runner_state, unused):
        ...
        def _env_step(runner_state, unused):
            train_state, pred_state, target_params,env_state, last_obs, obs_norm_params,rng = runner_state
            ...
            transition = RNDTransition(done, action, value, reward, int_reward,log_prob, last_obs, info)
            runner_state = (train_state, pred_state, target_params,env_state, obsv, obs_norm_params,rng)
        ...
        def _calculate_gae(traj_batch, last_val, rnd_int_return_norm_params):
            ...
            norm_int_reward, rnd_int_return_norm_params=rnd_normalise_int_rewards(traj_batch,rnd_int_return_norm_params,config["INT_GAMMA"])
            ...
            return advantages, advantages + traj_batch.value, norm_int_reward

        advantages, targets, norm_int_rewards = _calculate_gae(traj_batch, last_val,rnd_int_return_norm_params)

        # UPDATE NETWORK
        def _update_epoch(update_state, unused):

            train_state, pred_state, traj_batch, advantages, targets, obs_norm_params,rng = update_state
            ...
            obs_norm_params=update_obs_norm_params(obs_norm_params, traj_batch.obs.reshape(-1, init_obs.shape[-1]))
            # GET RND OBS
            rnd_obs = ((traj_batch.obs - obs_norm_params.mean)/jnp.sqrt(obs_norm_params.var)).clip(-5,5)
            ...
            (train_state,pred_state) , total_loss = jax.lax.scan(_update_minbatch, (train_state,pred_state), minibatches)
            update_state = (train_state, pred_state, traj_batch, advantages, targets, obs_norm_params,rng)
        ...
        runner_state = (train_state, pred_state, target_params,env_state, last_obs, obs_norm_params,rng)
        return runner_state, (metric, traj_batch.int_reward, norm_int_rewards, loss_info)

    rng, _rng = jax.random.split(rng)
    runner_state = (train_state, pred_state, target_params, env_state, obsv, obs_norm_params,_rng)
    ...
    return {
        ...
        "norm_int_rewards":norm_int_rewards,
...
for env_name in environments:
    ...
    if config["NUM_SEEDS"] > 1:
        ...
        rng, train_state,pred_state,target_params,init_rnd_obs = jax.vmap(ppo_make_train, out_axes=(1, 0))(rng)
