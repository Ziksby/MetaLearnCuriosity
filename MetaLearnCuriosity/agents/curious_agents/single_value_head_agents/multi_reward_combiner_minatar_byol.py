...
config = {
    "RUN_NAME": "DELETE_rc_cnn_delayed_norm_byol_ALL",
    "SEED": 42,
    "NUM_SEEDS": 2,
    "LR": 5e-3,
    "NUM_ENVS": 64,
...
def train(
    ...
    metric, _, norm_int_reward, int_reward, reward, int_lambdas, norm_ext_reward = extra_info
...
for env_name in environments:
    for step_int in step_intervals:
        es_stuff = Restore(
            "/home/batsy/MetaLearnCuriosity/MLC_logs/flax_ckpt/Reward_Combiners/Multi_task/rc_cnn_64_64_default_minatar_part2"
        )
        es_state, _ = es_stuff
        # print(es_state["mean"])
        rc_params = strategy.param_reshaper.reshape_single(es_state["mean"])
        # print(RewardCombiner().apply(rc_params, jnp.stack((jnp.zeros((1,128))*0.4,jnp.ones((1,128))), axis=-1)))
