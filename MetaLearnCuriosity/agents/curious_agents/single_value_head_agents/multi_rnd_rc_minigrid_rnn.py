...
def train(
    ...
    def _update_step(runner_state, _):
        ...
        def _env_step(runner_state, _):
            ...
            transition = RCRNDMiniGridTransition(
                done=done,
                action=action,
                value=value,
                reward=reward,
                norm_ext_reward=reward,
    ...
    metric, _, int_reward, norm_int_reward, int_lambdas, reward, norm_ext_reward = loss_info
    return {
...
for env_name in environments:
    config["RUN_NAME"] = f"RND_RC_RNN_{env_name}"
    es_state, _, _, _ = Restore(
        "/home/batsy/MetaLearnCuriosity/MLC_logs/flax_ckpt/Reward_Combiners/Multi_task/rnd_rc_rnn_empty_middle"
    )
    ...
    logger = WBLogger(
        ...
        tags=["reward_combiner", config["ENV_NAME"], "minigrid"],
